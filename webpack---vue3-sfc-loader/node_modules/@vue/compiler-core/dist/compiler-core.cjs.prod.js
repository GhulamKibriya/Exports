...
function genExpression(node, context) {
    ...
    context.push(
        isStatic ? JSON.stringify(content) : content, -3 /* Unknown */ ,
        node
    );
}
...
function genComment(node, context) {
    ...
    push(
        `${helper(CREATE_COMMENT)}(${JSON.stringify(node.content)})`, -3 /* Unknown */ ,
        node
    );
}
...
const transformElement = (node, context) => {
    return function postTransformElement() {
        ...
        node.codegenNode = createVNodeCall(
            ...
            vnodePatchFlag,
            vnodeDynamicProps,
            vnodeDirectives, !!shouldUseBlock,
            false,
            isComponent,
            node.loc
        );
                ...
                if (!hasDynamicKey) {
                    if (classProp && !isStaticExp(classProp.value)) {
                        classProp.value = createCallExpression(
                            context.helper(NORMALIZE_CLASS), [classProp.value]
                        );
                    }
                    if (styleProp && // the static style is compiled into an object,
                        ...
                        styleProp.value = createCallExpression(
                            context.helper(NORMALIZE_STYLE), [styleProp.value]
                        );
                    }
                } else {
                    propsExpression = createCallExpression(
                        context.helper(NORMALIZE_PROPS), [propsExpression]
                    );
                }
                break;
            case 14:
